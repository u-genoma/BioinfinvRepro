...
Luego, ejecutar el programa ‘IlluQC_PRLL.pl’, una de las herramientas de ‘NGSQC Toolkit’ y que tiene la funcionalidad de generar un reporte completo acerca de la calidad de las secuencias que reciba como entrada. La versión ‘PRLL’ permite ejecutar el proceso utilizando varios CPU al mismo tiempo. Para su conveniencia, hemos creado un alias para ejecutar el programa simplemente llamando ‘illuqc’ en el terminal.
...
Esto genera nuevas librerías de lecturas en formato ‘fastq’, las cuales serán utilizadas en el siguiente paso del tutorial. Investigue qué significan los argumentos utilizados y qué criterios se usaron para filtrar. Discuta si fue una buena elección según los resultados del QC y, si estima que existe una mejor elección, úsela en su informe, explicando la razón.
...
Nota: si usa el cluster, recuerde primero cargar un módulo de R para la versión que desee utilizar.
...
* Introducción
* Métodos (incluir códigos). Describir también los métodos y resultados de los puntos anteriores al 8, a pesar de que usted no los haya ejecutado.
* resultados y discusión (incluir de forma automática las figuras de los códigos en R usando Rmarkdown)
* Conclusiones
* Exporte a Markdown, versión GitHub, para incluir en su repositorio de GitHub.
* Incluya el archivo Rmarkdown en la misma carpeta del Markdown con su informe.
